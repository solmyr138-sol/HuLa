<template>
  <n-card title="企业信息">
    <n-descriptions v-if="info" :column="1" label-placement="left" bordered>
      ...
      <n-descriptions-item label="企业 LOGO">
        <img v-if="info.logo" :src="info.logo" alt="LOGO" class="logo-thumb" />
        <span v-else>未设置（社区页将显示企业名称）</span>
      </n-descriptions-item>
...
onMounted(async () => {
  info.value = await getTenantInfo()
})
</script>
