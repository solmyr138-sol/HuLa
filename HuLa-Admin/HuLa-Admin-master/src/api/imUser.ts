...
/** 后台 IM 注册用户（本租户） */
export interface AdminImUserItem {
  uid: number
  account: string
  mobile?: string
  nickname?: string
...
export interface AdminImUserPageParams {
  pageNo: number
  pageSize: number
  keyword?: string
  state?: number
...
export function getAdminImUserPage(params: AdminImUserPageParams): Promise<{
  pageNo: number
  pageSize: number
  totalRecords: number
  list: AdminImUserItem[]
}> {
  return request({
    url: '/admin/user/page',
    method: 'get',
